...
printf "\n\n" >> $LOG_FILE
echo $(nowstamp) Starting alfresco.bkp.full.aws.slow.sh...>> $LOG_FILE

echo $(nowstamp) Creating/Updating cron file >> $LOG_FILE
CRON_NAME=/etc/cron.d/alfresco-bkp-full-aws-slow
# Run ao 4AM on the first Sunday of every month
echo "00 04  * * 0 root  [ \"\$(date +\%d)\" -le 7 ] &&  $CURRENT/alfresco.bkp.full.aws.slow.sh 2>&1" > $CRON_NAME
